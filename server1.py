...
def Main():
    ...
    directory = '/home/bhargava/Academics/2-2/Assignments/Computer Networks/Assignment1/shared1'   # shared directory in server 2
    port = 5001                        # port num of this process
    s = socket.socket()
    s.bind((host,port))
    s.listen(1)
    c,addr = s.accept()
    ...
    while True:
        ...
        if message[0] == 'index_longlist':
            ...
            if os.path.exists(temp) and os.path.isfile(temp):
                timestamp = time.ctime(os.path.getmtime(temp))
                checksum = md5(temp) # hashlib.md5(open(temp,'rb').read()).hexdigest()
                error = 0
